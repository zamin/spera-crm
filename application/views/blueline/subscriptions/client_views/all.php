<div class="col-sm-12  col-md-12 main">  
     ...
     <div class="row">
		...
		<tr id="<?=$value->id;?>" >
			...
			<td class="hidden-xs"><span class="label <?php if( $value->status == "Active" && $value->next_payment > date('Y-m-d')){echo 'label-success';} if($value->next_payment < date('Y-m-d') && $value->status != "Inactive" && ($value->end_date > date('Y-m-d') || $value->end_date == "") ){ echo 'label-important';}?>" ><?php $unix = human_to_unix($value->next_payment.' 00:00'); echo '<span class="hidden">'.$unix.'</span> '; if(($value->end_date < date('Y-m-d') && $value->end_date != "") && $value->next_payment > date('Y-m-d')){ echo $this->lang->line('application_payments_closed');}else{echo date($core_settings->date_format, $unix);}?></span></td>
