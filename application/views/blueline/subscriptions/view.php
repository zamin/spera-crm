<div class="row">
		...
		<ul class="details col-xs-12 col-sm-6">
			...
			<li class="<?=$subscriptions[0]['status'];?>"><span><?=$this->lang->line('application_status');?>:</span>
			<a class="label <?php if($subscriptions[0]['status'] == 'Active'){ echo 'label-success';}else{echo 'label-important'; } ?>"><?php if(($subscriptions[0]['end_date'] <= date('Y-m-d') && $subscriptions[0]['end_date'] != "") && $subscriptions[0]['status'] != "Inactive"){ echo $this->lang->line('application_ended'); }else{ echo $this->lang->line('application_'.$subscriptions[0]['status']);}?></a>
			</li>
			<li><span><?=$this->lang->line('application_issue_date');?>:</span> <?php $unix = human_to_unix($subscriptions[0]['start_date'].' 00:00'); echo date($core_settings->date_format, $unix);?></li>
			<li><span><?=$this->lang->line('application_end_date');?>:</span> <?php if($subscriptions[0]['end_date'] != "" ){ ?><a class="label <?php if($subscriptions[0]['end_date'] <= date('Y-m-d') && $subscriptions[0]['status'] != "Inactive"){ echo 'label-success tt" title="'.$this->lang->line('application_subscription_has_ended'); } ?>"><?php $unix = human_to_unix($subscriptions[0]['end_date'].' 00:00'); echo date($core_settings->date_format, $unix);?></a> <?php }else{
			...
			if($subscriptions[0]['status'] == "Active" && $subscriptions[0]['next_payment'] > date('Y-m-d')){
				echo 'label-success ';}  ?>"><?php $unix = human_to_unix($subscriptions[0]['next_payment'].' 00:00');  
			if( $subscriptions[0]['next_payment'] > date('Y-m-d') ){ 
	...
	<div class="col-xs-12 col-sm-12">
		...
		<?php 
        foreach($subscriptionsInvoice as $value){?>

		<tr id="<?php echo $value->id;?>" >
            <td class="hidden-xs"><?php echo $value->id;?></td>
			<td><span><?php $unix = human_to_unix($value->start_date.' 00:00'); echo date($core_settings->date_format, $unix);?></span></td>
			<td><span class="label <?php if( strtoupper($value->result_value) == "SUCCESS"){echo 'label-success';} if($value->end_date <= date('Y-m-d') && strtoupper($value->result_value) != "SUCCESS"){ echo 'label-important tt" title="'.$this->lang->line('application_overdue'); } ?>"><?php $unix = human_to_unix($value->end_date.' 00:00'); echo date($core_settings->date_format, $unix);?></span></td>
            <td class="hidden-xs"><?php echo number_format($value->currency_converted_amount,2) ?> </td>
			<td><span class="label <?php $unix = human_to_unix($value->created_at.' 00:00'); if(strtoupper($value->result_value) == "SUCCESS"){echo 'label-success';}elseif(strtoupper($value->result_value) == "SUCCESS"){ echo 'label-warning tt" title="'.date($core_settings->date_format, $unix);} ?>"><?=$value->result_value;?></span></td>
		</tr>
