<?php if($conversation){ 
        ...
        foreach ($conversation as $value):
          ...
          if("s".$this->client->id == $value->sender){ $own = " own";}else{$own = "-previous";}
          $i = $i+1;
        ?>    
...
<?php if($i == "1" && $own != "own"){ ?>
 ...
 <div class="message-content-menu">
              ...
              <a class="btn btn-primary ajax-silent" href="<?php echo base_url(); ?>scmessages/mark/<?=$value->id?>" role="button">
              ...
              <?php 
              if(((substr($value->recipient, 1,9999) == $this->user->id) && ($value->receiver_delete != 1 )) || ((substr($value->sender, 1,9999) == $this->user->id) && ($value->status != "deleted")))
              { ?>
                  <a class="btn btn-danger" href="<?=base_url()?>scmessages/delete/<?=$value->id?>" role="button"><i class="fa fa-trash-o"></i> <?=$this->lang->line('application_delete');?></a>
                <?php
              }
              ?>
    ...
    <p class="subtitle"><?=$this->lang->line('application_from');?> <a href="#"><?php if(isset($value->sender_u) && $filter != "Deleted"){echo $value->sender_u;}else{ echo $value->sender_c; } ?></a> <?=$this->lang->line('application_to');?> <a href="#"><?php if(isset($recipient)){ echo $recipient; }else{if(isset($value->recipient_u)){echo $value->recipient_u;}else{ echo $value->recipient_c; } } ?></a>, <?=$this->lang->line('application_started_on');?> <?php  echo date($core_settings->date_format.' '.$core_settings->date_time_format, $unix); ?></p>
      ...
      <ul class="message-container">
          ...
          <?php   
                ...
                echo form_open_multipart(base_url().'scmessages/write/reply', $attributes); 
                ?>
       ...
       <?php 
              if($value->userpic_u){
                if($value->userpic_u != 'no-pic.png'){
                  echo site_url()."files/media/".$value->userpic_u;
                }else{
                ...
                if($value->userpic_c != 'no-pic.png'){
                  echo site_url()."files/media/".$value->userpic_c;
                }else{
      ...
      "/>
          ...
          <div class="attachments">
            <a class="label label-info" href="<?=base_url()?>scmessages/attachment/<?php echo $value->id; ?>"><?php echo $value->attachment; ?></a>
          </div>
            <?php } ?>
            </div>
        </li>
