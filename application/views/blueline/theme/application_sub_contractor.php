...
if(!$act_uri){ $act_uri = 'scdashboard'; }
...
<?php if(!empty($settings->login_logo)){ ?>
          ...
          <ul class="nav nav-sidebar">
              <?php foreach ($this->menu_data as $key => $value) { ?>
               <?php 
               if(strtolower($value->link) == "scmessages"){$message_icon = true;}
               ?>
               <li id="<?=strtolower($value->name);?>" class="<?php if ($act_uri == strtolower($value->link)) {echo "active";}?>"><a href="<?=base_url().$value->link.'/';?>"><span class="menu-icon"><i class="fa <?=$value->icon;?>"></i></span><span class="nav-text"><?php echo $this->lang->line('application_'.$value->link);?></span>
                <?php if(strtolower($value->link) == "scmessages" && $messages_new[0]->amount != "0"){ ?><span class="notification-badge"><?=$messages_new[0]->amount;?><?php //echo $messages_new[0]->amount;?></span><?php } ?>
                <?php if(strtolower($value->link) == "sctickets" && $this->tickets_assigned_note > 0 ){ ?><span class="notification-badge"><?=$this->tickets_assigned_note;?></span><?php } ?>
...
<a href="#" class="menu-trigger"><i class="ion-navicon visible-xs"></i></a>
            <?php if($message_icon){ ?>
              <span class="hidden-xs">
                  <a href="<?=site_url("scmessages");?>" title="<?=$this->lang->line('application_messages');?>">
