...
if(!$act_uri){ $act_uri = 'aodashboard'; }
    ...
    */?>
    <script type="">
        $( document ).ready(function() {
            ...
            $(".resend-invitaion-email").click(function(){
				$('.ajax-loader').show();
                var id = $(this).attr('data-id');
                var role = $(this).attr('data-role');
                $.ajax({
                    type: "POST",
                    url: "<?=base_url()?>subcontractors/invite/"+id,
                    data: "role="+role+"&<?php echo $this->security->get_csrf_token_name(); ?>=<?php echo $this->security->get_csrf_hash(); ?>",
                    success: function(data){
						$('.ajax-loader').hide();
                        var responseData = JSON.parse(data);
                        
                        var classname = 'success';
                        if(responseData.error == 1){classname = 'error';}
                        
                        $('#ajaxreturn').show();
                        $('#ajaxreturn').addClass(classname);
                        $('#ajaxreturn').velocity({
                            opacity: 1,
                            right: "10px",
                          }, 800, function() {
                            $('#ajaxreturn').delay( 3000 ).fadeOut();
                          });
                        $('#ajaxreturn').html(responseData.mail_sent);
                    },
                    complete: function (data) {
					  $('.ajax-loader').hide();
                      $(".chosen-select").chosen({scroll_to_highlighted: false, disable_search_threshold: 4, width: "100%"});
         ...
         <?php if(!empty($settings->login_logo)){ ?>
          ...
          <ul class="nav nav-sidebar">
              ...
              foreach ($this->menu_data as $key => $value) { ?>
               <?php 
               if(strtolower($value->link) == "aomessages"){$message_icon = true;}
               ?>
               <li id="<?=strtolower($value->name);?>" class="<?php if ($act_uri == strtolower($value->link)) {echo "active";}?>"><a href="<?=base_url().$value->link.'/';?>"><span class="menu-icon"><i class="fa <?=$value->icon;?>"></i></span><span class="nav-text"><?php echo $this->lang->line('application_'.$value->link);?></span>
                <?php if(strtolower($value->link) == "aomessages" && $messages_new[0]->amount != "0"){ ?><span class="notification-badge"><?=$messages_new[0]->amount;?></span><?php } ?>
                
                <?php if(strtolower($value->link) == "quotations" && ($quotations_new != "0" || $custom_quotation_new != "0")){ ?><span class="notification-badge"><?=$quotations_new + $custom_quotation_new ;?></span><?php } ?>
                <?php if(strtolower($value->link) == "aotickets" && $this->tickets_assigned_note > 0){ ?><span class="notification-badge"><?=$this->tickets_assigned_note;?></span><?php } ?>
               </a> </li>
              <?php } ?>
          </ul>
        </div>
...
<a href="#" class="menu-trigger"><i class="ion-navicon visible-xs"></i></a>
            <?php if($message_icon){ ?>
              <span class="hidden-xs">
                  <a href="<?=base_url()."aomessages";?>" title="<?=$this->lang->line('application_messages');?>">
    ...
    <!-- Notify -->
    <div class="notify"  id="ajaxreturn" style="display: none;"></div>
