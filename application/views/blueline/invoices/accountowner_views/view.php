<div class="row">
            ...
            <a href="<?=base_url()?>aoinvoices/update/<?=$invoice->id;?>/view" class="btn btn-primary" data-toggle="mainmodal"><i class="fa fa-edit visible-xs"></i><span class="hidden-xs"><?=$this->lang->line('application_edit_invoice');?></span></a>
			<?php if($invoice->estimate_status != "Invoiced"){ ?><a href="<?=base_url()?>aoinvoices/item/<?=$invoice->id;?>" class="btn btn-primary" data-toggle="mainmodal"><i class="fa fa-plus visible-xs"></i><span class="hidden-xs"><?=$this->lang->line('application_add_item');?></span></a><?php } ?>
            <a href="<?=base_url()?>aoinvoices/payment/<?=$invoice->id;?>" class="btn btn-primary" data-toggle="mainmodal"><i class="fa fa-credit-card visible-xs"></i><span class="hidden-xs"><?=$this->lang->line('application_add_payment');?></span></a>
			...
			<div class="btn-group">
			  ...
			  <ul class="dropdown-menu">
			    <li><a href="<?=base_url()?>aoinvoices/preview/<?=$invoice->id;?>" ><?=$this->lang->line('application_download_pdf');?></a></li>
			    <li><a href="<?=base_url()?>aoinvoices/preview/<?=$invoice->id;?>/show" target="_blank"><?=$this->lang->line('application_preview_pdf');?></a></li>
			  </ul>
			</div>
			<?php if($invoice->status != "Paid" && isset($invoice->company->name)){ ?><a href="<?=base_url()?>aoinvoices/sendinvoice/<?=$invoice->id;?>" class="btn btn-primary"><i class="fa fa-envelope visible-xs"></i><span class="hidden-xs"><?=$this->lang->line('application_send_invoice_to_client');?></span></a><?php } ?>
			...
			foreach ($payments as $value) {  ?>
				...
				<tr class="sec">
					<td class="option" style="text-align:left;" width="8%">
					        ...
					        <button type="button" class="btn-option delete po" data-toggle="popover" data-placement="right" data-content="<a class='btn btn-danger po-delete ajax-silent' href='<?=base_url()?>aoinvoices/payment_delete/<?=$payments[$i]->id;?>/<?=$invoice->id;?>'><?=$this->lang->line('application_yes_im_sure');?></a> <button class='btn po-close'><?=$this->lang->line('application_no');?></button> <input type='hidden' name='td-id' class='id' value='<?=$value->id;?>'>" data-original-title="<b><?=$this->lang->line('application_really_delete');?></b>"><i class="fa fa-times"></i></button>
					        <a href="<?=base_url()?>aoinvoices/payment_update/<?=$payments[$i]->id;?>" title="<?=$this->lang->line('application_edit');?>" class="btn-option" data-toggle="mainmodal"><i class="fa fa-cog"></i></a>
		...
		<div class=" col-md-12" align="right">
			<?php if($core_settings->paypal == "1" && $sum != "0.00" && $invoice->status != "Paid"){ 
				...
				?>
						...
						<input type="hidden" name="return" value="<?=base_url()?>aoinvoices/view/<?=$invoice->id;?>"> 
						<input type="hidden" name="cancel_return" value="<?=base_url()?>aoinvoices/view/<?=$invoice->id;?>">
	...
	<div class="btn-group dropup">
	  ...
	  <?php if($core_settings->bank_transfer == "1" && $sum != "0.00" && $invoice->status != "Paid" ){ ?>
	    <li><a id="pay_bank_transfer" data-toggle="mainmodal" href="<?=base_url()?>aoinvoices/banktransfer/<?=$invoice->id;?>/<?=$sumRest;?>"><i class="fa fa-money" style="margin-right:5px"></i>  <?=$this->lang->line('application_bank_transfer');?></a></li>
	  <?php } ?>
	  ...
	  <?php if($core_settings->stripe == "1" && $core_settings->twocheckout == "0" && $core_settings->authorize_net == "0" && $sum != "0.00" && $invoice->status != "Paid" ){ ?>  
	    ...
	    <li><a id="pay_credit_card" data-toggle="mainmodal" href="<?=base_url()?>aoinvoices/stripepay/<?=$invoice->id;?>/<?=$sumRest;?>"><i class="fa fa-credit-card" style="margin-right:5px"></i> <?=$this->lang->line('application_credit_card');?></a></li>
	  <?php } ?>
	  ...
	  <?php if($core_settings->twocheckout == "1" && $core_settings->stripe == "0" && $core_settings->authorize_net == "0" && $sum != "0.00" && $invoice->status != "Paid" ){ ?>  
	    ...
	    <li><a id="pay_credit_card" data-toggle="mainmodal" href="<?=base_url()?>aoinvoices/twocheckout/<?=$invoice->id;?>/<?=$sumRest;?>"><i class="fa fa-credit-card" style="margin-right:5px"></i> <?=$this->lang->line('application_credit_card');?></a></li>
	  <?php } ?>

	  <?php if($core_settings->stripe == "0" && $core_settings->twocheckout == "0" && $core_settings->authorize_net == "1" && $sum != "0.00" && $invoice->status != "Paid" ){ ?>  
	    <li><a id="pay_credit_card" data-toggle="mainmodal" href="<?=base_url()?>aoinvoices/authorizenet/<?=$invoice->id;?>/<?=$sumRest;?>"><i class="fa fa-credit-card" style="margin-right:5px"></i> <?=$this->lang->line('application_credit_card');?></a></li>
	  <?php } ?>
	  </ul>
	</div>
