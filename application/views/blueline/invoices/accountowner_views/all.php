	...
	<a href="<?=base_url()?>aoinvoices/create" class="btn btn-primary" data-toggle="mainmodal"><?=$this->lang->line('application_create_invoice');?></a>

       <div class="btn-group pull-right-responsive margin-right-3">
          <button type="button" class="btn btn-primary dropdown-toggle" data-toggle="dropdown">
            <?php   //$last_uri = $this->uri->segment($this->uri->total_segments()); 
					$last_uri = $this->uri->segment(1);
					if($last_uri != "invoices") {
      ...
      <div class="row">
		...
		<thead>
			...
			<th><?=$this->lang->line('application_project');?></th>
		...
		<tr id="<?=$value['id'];?>" >
			...
			<td><span class="label label-info"><?php if(isset($value['prname'])){echo $value['prname']; }?></span></td>
			...
			<td class="option" width="8%">
				<button type="button" class="btn-option delete po" data-toggle="popover" data-placement="left" data-content="<a class='btn btn-danger po-delete ajax-silent' href='<?=base_url()?>aoinvoices/delete/<?=$value['id'];?>'><?=$this->lang->line('application_yes_im_sure');?></a> <button class='btn po-close'><?=$this->lang->line('application_no');?></button> <input type='hidden' name='td-id' class='id' value='<?=$value['id'];?>'>" data-original-title="<b><?=$this->lang->line('application_really_delete');?></b>"><i class="fa fa-times"></i></button>
				<a href="<?=base_url()?>aoinvoices/update/<?=$value['id'];?>" class="btn-option" data-toggle="mainmodal"><i class="fa fa-cog"></i></a>
			</td>
