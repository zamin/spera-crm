<div class="row">
			...
			<?php if($invoice->estimate_status != "Invoiced"){ ?><a href="<?=base_url()?>invoices/item/<?=$invoice->id;?>" class="btn btn-primary" data-toggle="mainmodal"><i class="fa fa-plus visible-xs"></i><span class="hidden-xs"><?=$this->lang->line('application_add_task');?></span></a><?php } ?>
			...
			<?php if($invoice->status != "Paid" && isset($invoice->company->name) && isset($selected_invoice_user['name'])){ ?>
				<a href="<?=base_url()?>invoices/sendinvoice/<?=$invoice->id;?>" class="btn btn-primary"><i class="fa fa-envelope visible-xs"></i><span class="hidden-xs"><?=$this->lang->line('application_send_invoice_to_client');?></span></a>
			<?php }else{ ?>
                <a href="<?=base_url()?>invoices/refundpayment/<?=$invoice->id;?>" class="btn btn-primary"><i class="fa fa-envelope visible-xs"></i><span class="hidden-xs"><?=$this->lang->line('application_refund');?></span></a>
            <?php }?>
		...
		<ul class="details col-xs-12 col-sm-6">
			...
			<li><span><?=$this->lang->line('application_vat');?>:</span> <?php echo $company_detail->vat; ?></li>
			<?php } ?>
			<?php /* if(isset($invoice->project->name)){?>
			<li><span><?=$this->lang->line('application_projects');?>:</span> <?php echo $invoice->project->name; ?></li>
			<?php } */ ?>
			<li><span><?=$this->lang->line('application_project');?>:</span> <a href="<?=base_url()?>aoprojects/view/<?=$invoice->project_id;?>" class="label label-info"><?=$invoice->project_name;?></a></li>
			<?php if(isset($selected_invoice_user['name'])){?>
			<li><span><?=$this->lang->line('application_clients');?>:</span><?=implode(', ',$selected_invoice_user['name']);?></li>
			<?php } ?>
			<span class="visible-xs"></span>
		</ul>
		<ul class="details col-xs-12 col-sm-6">
			<?php if(isset($invoice->company->name)){ ?>
			<li><span><?=$this->lang->line('application_company');?>:</span><?=$invoice->company->name;?></li>
			<?php /* ?>
			...
			<?php */ ?>
			<li><span><?=$this->lang->line('application_street');?>:</span> <?php echo (isset($company_detail->address)) ? $company_detail->address : '';?></li>
			<li><span><?=$this->lang->line('application_city');?>:</span> <?php echo (isset($company_detail->zipcode)) ? $company_detail->zipcode : '';?> <?=(isset($company_detail->city)) ? $company_detail->city : '';?></li>
			<li><span><?=$this->lang->line('application_province');?>:</span> <?php echo (isset($company_detail->state)) ? $company_detail->state : "-" ; ?></li>
			<?php }else{ ?>
		...
		<th width="4%"><?=$this->lang->line('application_action');?></th>
			...
			<th class="hidden-xs" width="8%"><?=$this->lang->line('application_hours').':'.$this->lang->line('application_minutes');?></th>
		...
		<tr id="<?=$value->id;?>" >
			...
			<?php 
			$time_spent_value = explode('.', $invoice->invoice_has_items[$i]->amount);
			if(count($time_spent_value) == 2) {
				$time_spent = sprintf("%02d:%02d", $time_spent_value[0], $time_spent_value[1]);
				$time_spent_float = sprintf("%02d.%02d", $time_spent_value[0], $time_spent_value[1]);
			} else if(count($time_spent_value) == 1) {
				$time_spent = sprintf("%02d:%02d", $time_spent_value[0], 0);
				$time_spent_float = sprintf("%02d.%02d", $time_spent_value[0], 0);
			} else {
				$time_spent = '00:00';
				$time_spent_float = 0;
			}
			...
			<td class="hidden-xs" align="center"><?=$time_spent;?></td>
			<td class="hidden-xs"><?php echo display_money(sprintf("%01.2f",$invoice->invoice_has_items[$i]->value));?></td>
			<?php /* ?><td class="hidden-xs"><?php echo display_money(sprintf("%01.2f",$invoice->invoice_has_items[$i]->amount*$invoice->invoice_has_items[$i]->value));?></td> <?php */ ?>
			<td class="hidden-xs"><?php echo display_money(sprintf("%01.2f",$time_spent_float*$invoice->invoice_has_items[$i]->value));?></td>
	...
	<div class="btn-group dropup">
	  ...
	  <?php if($core_settings->twocheckout == "1" && $core_settings->stripe == "0" && $core_settings->authorize_net == "0" && $sum != "0.00" && $invoice->status != "Paid" ){ ?>  
	    ...
	    <li><a id="pay_credit_card" data-toggle="mainmodal" href="<?=base_url()?>invoices/twocheckout/<?=$invoice->id;?>/<?=$sumRest;?>"><i class="fa fa-credit-card" style="margin-right:5px"></i> <?=$this->lang->line('application_credit_card');?></a></li>
	  <?php } ?>
