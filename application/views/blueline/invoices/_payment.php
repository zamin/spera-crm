...
<?php } ?>
 ...
 <div class="form-group">
        ...
        <input id="value" type="text" name="amount" class="required form-control number money-format"  value="<?php if(isset($payment)){ echo $payment->amount; }else{ echo $sumRest; }?>" required/>
 </div>
 <div class="form-group">
                          <label for="date"><?=$this->lang->line('application_date');?> *</label>
                          <input class="form-control datepicker" name="date" id="date" type="text" value="<?php if(isset($payment)){ echo $payment->date; }else{  echo date('Y-m-d', time()); }?>" required/>
</div>
<div class="form-group">
        <label for="client"><?=$this->lang->line('application_type');?></label><br>
        <?php $options = array();
