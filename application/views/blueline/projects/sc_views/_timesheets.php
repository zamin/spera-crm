    ...
    <?php echo date($core_settings->date_format, strtotime($value->end)); ?>     
                    ...
                    <td>
                        <?php if ($this->user->id == $value->user_id) { ?>
                            <a href="<?= base_url() ?>scprojects/timesheet_delete/<?= $value->id; ?>" class="deleteThisRow ajax-silent" title="<?= $this->lang->line('application_delete'); ?>"><i class="ion-close-circled red"></i></a>    
    <?php } ?>
                    </td>
...
<?php endforeach; ?>
        ...
        <td>
            <a href="#" class="add-row-ajax" title="<?= $this->lang->line('application_save'); ?>"><i class="ion-plus-circled"></i></a> <span class="delete_link hidden"><?= base_url() ?>scprojects/timesheet_delete/</span>
...
<div class="modal-footer">
    <a id="sc_timesheet_submit" class="btn btn-success" href="javascript:;"><?= $this->lang->line('application_save'); ?></a>
...
<script>
    $(document).ready(function () {
        $('#sc_timesheet_submit').click(function () {
            var hours = $('#hours').val();
            var minutes = $('#minutes').val();
            var project_id = $('#project_id').val();
            var redirect_url = "<?php echo base_url(); ?>scprojects/view/" + project_id + "#tasks-tab";
            if (hours != '00' || minutes != '00')
            {
                ...
                var ajax_url = "<?php echo base_url(); ?>scprojects/timesheet_add_by_button";
                $.ajax({
                    type: "GET",
                    url: ajax_url,
                    cache:false,
