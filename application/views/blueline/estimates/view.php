          ...
          <div class="row">
			...
			<?php if(($estimate->estimate_status == "Open" || $estimate->estimate_status == "Revised") && isset($estimate->company->name) && isset($selected_invoice_user['name'])){ ?>
				<a href="<?=base_url()?>estimates/sendestimate/<?=$estimate->id;?>" class="btn btn-primary"><i class="fa fa-envelope visible-xs"></i><span class="hidden-xs"><?=$this->lang->line('application_send_estimate_to_client');?></span></a>
			<?php } ?>
		...
		<ul class="details col-xs-12 col-sm-6">
			...
			<li><span><?=$this->lang->line('application_company');?>:</span><?=$estimate->company->name;?></li>
			<?php /* ?>
