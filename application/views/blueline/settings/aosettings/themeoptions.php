<div id="row">
		<div class="col-md-3">
			<div class="list-group">
				...
				$badge = "";
				$active = "";
				if($value == "aosettings/updates"){ $badge = '<span class="badge badge-success">'.$update_count.'</span>';}
        ...
        <div class="form-group">
			<label><?=$this->lang->line('application_login');?> <?=$this->lang->line('application_logo');?>
				...
				<?php if($settings->login_logo != ""){ ?>
						<button type="button" class="btn-option po " data-toggle="popover" data-placement="right"  data-content="<div style='padding:10px'><img src='<?=site_url();?><?=$settings->login_logo;?>' width='196' height='33' ></div>" data-original-title="<?=$this->lang->line('application_login');?> <?=$this->lang->line('application_logo');?>"> <i class="ion-eye"></i></button>
						<?php }else{ ?>
                                                <button type="button" class="btn-option po " data-toggle="popover" data-placement="right"  data-content="<div style='padding:10px'><img src='<?=site_url();?><?=$core_settings->login_logo;?>'></div>" data-original-title="<?=$this->lang->line('application_login');?> <?=$this->lang->line('application_logo');?>"> <i class="ion-eye"></i></button>
                                                <?php } ?>
			</label>
	                <div>
       ...
       <!--  <div class="form-group">
		...
		<div class="image-page-frame">
                        <?php if($settings->login_style==''){ ?>
                            <img class="hidden-xs" id="login_style_left" style="max-height:200px" src="<?=site_url()?>assets/blueline/images/login_side.jpg">
                        <?php } ?>
			<img class="hidden-xs <?php if($settings->login_style != "left"){ echo "hidden"; }?>" id="login_style_left" style="max-height:200px" src="                        <?=site_url()?>assets/blueline/images/login_side.jpg">
			<img class="hidden-xs <?php if($settings->login_style != "center"){ echo "hidden"; }?>" id="login_style_center" style="max-height:130px; margin-top:25px; margin-left: 110px; position:absolute;" src="<?=site_url()?>assets/blueline/images/login_side.jpg">
                        <?php if($settings->login_background==''){ ?>
			<img class="hidden-xs" id="background_preview" style="max-height:200px" src="<?=site_url()?>assets/blueline/images/backgrounds/<?=$core_settings->login_background;?>">
			<img class="visible-xs img-responsive" src="<?=site_url()?>assets/blueline/images/backgrounds/<?=$core_settings->login_background;?>">
                        <?php }else{ ?>
                        <img class="hidden-xs" id="background_preview" style="max-height:200px" src="<?=site_url()?>assets/blueline/images/backgrounds/<?=$settings->login_background;?>">
			<img class="visible-xs img-responsive" src="<?=site_url()?>assets/blueline/images/backgrounds/<?=$settings->login_background;?>">
                        <?php }?>
                </div>
...
<script>
	$('#login_background').on('change', function(evt, params) {
			$("#background_preview").attr("src","<?=site_url()?>assets/blueline/images/backgrounds/"+params.selected);
	 });
	$('#login_style').on('change', function(evt, params) {
                        var login_style=$(this).val();
                        
                        if(login_style=='left')
                        {
                            $("#login_style_left").removeClass('hidden');
                            $("#login_style_center").addClass('hidden');
                        }
                        else
                        {
                           $("#login_style_left").addClass('hidden');
                           $("#login_style_center").removeClass('hidden');
                        }
	 });
