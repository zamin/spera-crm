     ...
     <div class="row">
      ...
      <div class="row">
         ...
         <div class="table-div">
			...
			<tr id="<?=$value['invoiceid'];?>" >
				<td class="hidden-xs"><?=$value['invoiceid'];?></td>
				...
				<td class="hidden-xs"><span class="label <?php if( strtoupper($value['result_value']) == "SUCCESS"){echo 'label-success';} if($value['end_date'] <= date('Y-m-d') && strtoupper($value['result_value']) != "SUCCESS"){ echo 'label-important tt" title="'.$this->lang->line('application_overdue'); } ?>"><?php $unix = human_to_unix($value['end_date'].' 00:00'); echo '<span class="hidden">'.$unix.'</span> '; echo date($core_settings->date_format, $unix);?></span> <span class="hidden"><?=$unix;?></span></td>
				<td class="hidden-xs"><?php echo number_format($value['amount'],2) ?> </td>
			    <td ><span class="label"><?=strtoupper($value['result_value']);?></span></td>
