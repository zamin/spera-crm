...
<div class="col-xs-12 col-sm-12">
	<div class="table-head">Task Comment Details
<span class=" pull-right btn-backp"><a class="btn btn-primary" href="<?=base_url()?><?=$backlink;?>"><i class="fa fa-arrow-left"></i> <?=$this->lang->line('application_back_to_project');?></a><a class="btn btn-primary open-comment-box"><?=$this->lang->line('application_new_comment');?></a></span>  </div> 
  <div class="subcont" > 
  <ul id="comments-ul" class="comments">
    <li class="comment-item add-comment">
      <?php   
                ...
                echo form_open(base_url().'scprojects/tasks/'.$task->project_id.'/comment/'.$task->id, $attributes); 
                ?>
  ...
  <?php foreach ($task_comments as $value):?>
      <?php 
      $writer = FALSE;
      if ($value['user_id'] != 0) { 
