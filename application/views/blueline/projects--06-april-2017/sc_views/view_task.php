 ...
 <script type="text/javascript" src="<?=site_url()?>assets/blueline/js/plugins/pdfobject.js"></script>
       ...
       <div class="col-xs-12 col-sm-6">
		...
		<div class="subcont">
				...
				<!-- <?php 
				    if(!empty($task_attachments))
				    {
				        foreach ($task_attachments as $k => $v) {
				           ...
				           if(file_exists($path))
				           {
				              //echo "<a href='".$attchment_url."' download='".$v['task_attach_file']."'>Download</a><br/>";
				              echo "<li><span>".$this->lang->line('application_download').":</span><a href='".$attchment_url."' download='".$v['task_attach_file']."' class='btn btn-xs btn-success'><i class='icon-download icon-white'>".$this->lang->line('application_download')."</i></a>&nbsp;&nbsp;&nbsp;<a href='".base_url().'scprojects/delete_tasks_attachement/'.$v['project_id'].'/delete/'.$v['task_id'].'/'.$v['id']."' class='btn btn-xs btn-error' >Delete</a><br/></li>";
				           }
				        }
				    }
				?> -->
			...
			<br clear="both">
			<form class="dynamic-form" method='POST' action="<?php echo base_url();?>scprojects/tasks/<?php echo $task->project_id;?>/view/<?php echo $task->id;?>" enctype ="multipart/form-data" >
    	 <input type="hidden" name="<?php echo $this->security->get_csrf_token_name(); ?>" value="<?php echo $this->security->get_csrf_hash(); ?>"  >

    	 	<div class="form-group">
		    	<label for="task_attach_file">File</label>
		    	<div>
				    ...
				    if(!empty($task_attachments))
				    {
				        foreach ($task_attachments as $k => $v) {
				            ...
				            if(file_exists($path))
				            {
				            	...
				            	echo "<td><a href='".$attchment_url."' download='".$v['task_attach_file']."' class='btn btn-xs btn-success'>".$this->lang->line('application_download')."</a>&nbsp;&nbsp;&nbsp;<a href='".base_url().'scprojects/delete_tasks_attachement/'.$v['project_id'].'/delete/'.$v['task_id'].'/'.$v['id']."' class='btn btn-xs btn-error' >Delete</a></td>";
				              echo "</tr>";
				            }
