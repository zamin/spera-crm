 ...
 * This class is used for client messages
 **/
if (!defined('BASEPATH'))
  ...
  function __construct()
    ...
    function messagelist($con = FALSE, $deleted = FALSE)
    {
        ...
        $sql = 'SELECT privatemessages.id, privatemessages.`status`, privatemessages.`deleted`, privatemessages.attachment, privatemessages.attachment_link, privatemessages.subject, privatemessages.conversation, privatemessages.sender, privatemessages.recipient, privatemessages.message, privatemessages.`time`, users.`userpic` as userpic_c, users.`userpic` as userpic_u , users.`email` as email_u , users.`email` as email_c , CONCAT(users.firstname," ", users.lastname) as sender_u, CONCAT(users.firstname," ", users.lastname) as sender_c
                ...
                GROUP by privatemessages.id HAVING privatemessages.recipient = "u'.$this->user->id.'" OR privatemessages.recipient = "c'.$this->user->id.'" OR privatemessages.recipient = "s'.$this->user->id.'" '.$qdeleted.' ORDER BY privatemessages.`time` DESC LIMIT '.$limit.$max_value.'';
        ...
        $sql2 = 'SELECT privatemessages.id, privatemessages.`status`, privatemessages.`deleted`, privatemessages.attachment, privatemessages.attachment_link, privatemessages.subject, privatemessages.conversation, privatemessages.sender, privatemessages.recipient, privatemessages.message, privatemessages.`time`, users.`userpic` as userpic_c, users.`userpic` as userpic_u , users.`email` as email_u , users.`email` as email_c , CONCAT(users.firstname," ", users.lastname) as sender_u, CONCAT(users.firstname," ", users.lastname) as sender_c
                ...
                LEFT JOIN users ON CONCAT("u",users.id) = privatemessages.sender OR CONCAT("c",users.id) = privatemessages.sender OR CONCAT("s",users.id) = privatemessages.sender
                WHERE privatemessages.receiver_delete != 1
                GROUP by privatemessages.id HAVING privatemessages.recipient = "u'.$this->user->id.'" OR privatemessages.recipient = "c'.$this->user->id.'" OR privatemessages.recipient = "s'.$this->user->id.'" '.$qdeleted.' ORDER BY privatemessages.`time` DESC';
    ...
    function filter($condition = FALSE, $con = FALSE)
    {
        ...
        switch ($condition) {
            case 'read':
                ...
                break;
            case 'sent':
                $sql = 'SELECT privatemessages.id, privatemessages.`status`, privatemessages.subject, privatemessages.attachment, privatemessages.attachment_link, privatemessages.message, privatemessages.sender, privatemessages.recipient, privatemessages.receiver_delete, privatemessages.`time`, users.`userpic` as userpic_c, users.`userpic` as userpic_u , users.`email` as email_u , users.`email` as email_c , CONCAT(users.firstname," ", users.lastname) as sender_u, CONCAT(users.firstname," ", users.lastname) as sender_c
                FROM privatemessages
                ...
                //echo '<br>';
                $sql2 = 'SELECT privatemessages.id, privatemessages.`status`, privatemessages.subject, privatemessages.attachment, privatemessages.attachment_link, privatemessages.conversation, privatemessages.sender, privatemessages.recipient, privatemessages.receiver_delete, privatemessages.message, privatemessages.`time`, users.`userpic` as userpic_c, users.`userpic` as userpic_u , users.`email` as email_u , users.`email` as email_c , CONCAT(users.firstname," ", users.lastname) as sender_u, CONCAT(users.firstname," ", users.lastname) as sender_c
                FROM privatemessages
                LEFT JOIN users ON CONCAT("u",users.id) = privatemessages.recipient 
                where privatemessages.deleted != 1
            ...
            case 'marked':
                ...
                break;
          	case 'deleted':
          	  	$sql = 'SELECT privatemessages.id, privatemessages.`status`, privatemessages.`deleted`, privatemessages.attachment, privatemessages.attachment_link, privatemessages.subject, privatemessages.conversation, privatemessages.sender, privatemessages.recipient, privatemessages.message, privatemessages.receiver_delete, privatemessages.`time`, users.`userpic` as userpic_c, users.`userpic` as userpic_u , users.`email` as email_u , users.`email` as email_c , CONCAT(users.firstname," ", users.lastname) as sender_u, CONCAT(users.firstname," ", users.lastname) as sender_c
                FROM privatemessages
                LEFT JOIN users ON CONCAT("u",users.id) = privatemessages.sender 
                WHERE (privatemessages.recipient = "u'.$this->user->id.'" AND privatemessages.receiver_delete = 1) OR (privatemessages.sender = "u'.$this->user->id.'" AND privatemessages.status = "deleted" AND privatemessages.deleted = 1 )
        ...
        $rows = $query2->num_rows();
        $this->view_data["message"] = array_filter($query->result());
        //print_r($this->view_data["message"]);
        $this->view_data["message_rows"] = $rows;
        $this->view_data["message_list_page_next"] = $con+$max_value;
        $this->view_data["message_list_page_prev"] = $con-$max_value;
    ...
    {   
        if($_POST){
            ...
            if(!$message){$this->session->set_flashdata('message', 'error:'.$this->lang->line('messages_write_message_error'));}
                    ...
                    /*$this->load->helper('notification');
                    $responce = send_notification($receiveremail, $this->lang->line('application_notification_new_message_subject'), $this->lang->line('application_notification_new_message').'<br><hr style="border-top: 1px solid #CCCCCC; border-left: 1px solid whitesmoke; border-bottom: 1px solid whitesmoke;"/>'.$_POST['message'].'<hr style="border-top: 1px solid #CCCCCC; border-left: 1px solid whitesmoke; border-bottom: 1px solid whitesmoke;"/>', $attachment);*/
            ...
            if($ajax != "reply"){ redirect('cmessages'); }
            else{
                    $this->theme_view = 'ajax';
                }
            redirect('cmessages/');
        ...
        {
            $clients = $this->db->query('SELECT DISTINCT(u.id), u.firstname, u.lastname, r.role_id FROM users u JOIN user_roles r ON u.id = r.user_id WHERE (r.role_id =2 OR r.role_id =3 OR r.role_id = 4) and r.company_id="'.$this->sessionArr['company_id'].'" and u.status="active"')->result_array();
                ...
                foreach($clients as $newclient)
                {
                   if($newclient['role_id']==4)
                    ...
                    else
                    {
                        ...
                        else if($this->user->id==$newclient['id'])
                        {
            ...
            $this->view_data['form_action'] = base_url().'cmessages/write';
    ...
    {   
        if($_POST){
            ...
            if(isset($view)){redirect('messages/client/view/'.$id);}else{redirect('messages');}
        ...
        }else
        {
            ...
            $this->view_data['form_action'] = 'messages/client/update';
    ...
    function view($id = FALSE, $filter = FALSE, $additional = FALSE)
    {
        ...
        //echo 'reach';die;
            ...
            $query = $this->db->query($sql);
            $row = $query->row();   
            $sql2 = 'SELECT privatemessages.id, privatemessages.`status`, privatemessages.deleted, privatemessages.conversation, privatemessages.attachment, privatemessages.attachment_link, privatemessages.subject, privatemessages.message, privatemessages.sender, privatemessages.receiver_delete, privatemessages.recipient, privatemessages.`time`, privatemessages.`sender` , u1.`userpic` as userpic_c, users.`userpic` as userpic_u , users.`email` as email_u , u1.`email` as email_c , CONCAT(users.firstname," ", users.lastname) as sender_u, CONCAT(u1.firstname," ", u1.lastname) as sender_c, CONCAT(rec_u.firstname," ", rec_u.lastname) as recipient_u, CONCAT(rec_c.firstname," ", rec_c.lastname) as recipient_c
                    FROM privatemessages
        ...
        else
        {
            ...
            {
            	//echo 'asdf';die;
            	$sql = 'SELECT privatemessages.id, privatemessages.`status`, privatemessages.deleted, privatemessages.conversation, privatemessages.attachment, privatemessages.attachment_link, privatemessages.subject, privatemessages.message, privatemessages.sender, privatemessages.receiver_delete, privatemessages.recipient, privatemessages.`time`, privatemessages.`sender` , clients.`userpic` as userpic_c, users.`userpic` as userpic_u , users.`email` as email_u , clients.`email` as email_c , CONCAT(users.firstname," ", users.lastname) as sender_u, CONCAT(clients.firstname," ", clients.lastname) as sender_c, CONCAT(users.firstname," ", users.lastname) as recipient_u, CONCAT(clients.firstname," ", clients.lastname) as recipient_c
                    FROM privatemessages
                    LEFT JOIN users AS clients ON (CONCAT("u",clients.id) = privatemessages.recipient)
                    LEFT JOIN users ON (CONCAT("u",users.id) = privatemessages.sender) 
                    GROUP by privatemessages.id HAVING privatemessages.id = "'.$id.'" AND (privatemessages.recipient = "u'.$this->user->id.'" OR privatemessages.sender = "u'.$this->user->id.'") ORDER BY privatemessages.`id` DESC LIMIT 100';
            ...
            else
            {
            	//echo 'reasdf';die;
                if($filter == "Sent"){
                     $sql = 'SELECT privatemessages.id, privatemessages.`status`, privatemessages.deleted, privatemessages.conversation, privatemessages.attachment, privatemessages.attachment_link, privatemessages.subject, privatemessages.message, privatemessages.receiver_delete, privatemessages.sender, privatemessages.recipient, privatemessages.`time`, privatemessages.`sender` , u1.`userpic` as userpic_c, users.`userpic` as userpic_u , users.`email` as email_u , u1.`email` as email_c , CONCAT(users.firstname," ", users.lastname) as sender_u, CONCAT(u1.firstname," ", u1.lastname) as sender_c, CONCAT(users.firstname," ", users.lastname) as recipient_u, CONCAT(u1.firstname," ", u1.lastname) as recipient_c
                    FROM privatemessages
                    ...
                    {
                    	$sql = 'SELECT privatemessages.id, privatemessages.`status`, privatemessages.deleted,privatemessages.conversation, privatemessages.attachment, privatemessages.attachment_link, privatemessages.subject, privatemessages.receiver_delete, privatemessages.message, privatemessages.sender, privatemessages.recipient, privatemessages.`time`, privatemessages.`sender` , clients.`userpic` as userpic_c, users.`userpic` as userpic_u , users.`email` as email_u , clients.`email` as email_c , CONCAT(users.firstname," ", users.lastname) as sender_u, CONCAT(users.firstname," ", users.lastname) as sender_c, CONCAT(clients.firstname," ", clients.lastname) as recipient_u, CONCAT(clients.firstname," ", clients.lastname) as recipient_c
                    FROM privatemessages
                    ...
                    LEFT JOIN users ON (CONCAT("u",users.id) = privatemessages.sender) OR (CONCAT("c",users.id) = privatemessages.sender) OR (CONCAT("s",users.id) = privatemessages.sender)
                    GROUP by privatemessages.id HAVING privatemessages.conversation = "'.$message->conversation.'" ORDER BY privatemessages.`id` DESC LIMIT 100';
                    }
                    else
                    {
                    	$sql = 'SELECT privatemessages.id, privatemessages.`status`, privatemessages.deleted, privatemessages.conversation, privatemessages.attachment, privatemessages.attachment_link, privatemessages.receiver_delete, privatemessages.subject, privatemessages.message, privatemessages.sender, privatemessages.recipient, privatemessages.`time`, privatemessages.`sender` , clients.`userpic` as userpic_c, users.`userpic` as userpic_u , users.`email` as email_u , clients.`email` as email_c , CONCAT(users.firstname," ", users.lastname) as sender_u, CONCAT(users.firstname," ", users.lastname) as sender_c, CONCAT(clients.firstname," ", clients.lastname) as recipient_u, CONCAT(clients.firstname," ", clients.lastname) as recipient_c
                    FROM privatemessages
                    LEFT JOIN users AS clients ON (CONCAT("c",clients.id) = privatemessages.recipient) OR (CONCAT("s",clients.id) = privatemessages.recipient) OR (CONCAT("u",clients.id) = privatemessages.recipient)
                    LEFT JOIN users ON (CONCAT("u",users.id) = privatemessages.sender) OR (CONCAT("c",users.id) = privatemessages.sender) OR (CONCAT("s",users.id) = privatemessages.sender)
