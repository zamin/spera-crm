 ...
 * ClassName: Dashboard
 * Function Name: index 
 * This class is used for Main Dashboard
 **/
if (!defined('BASEPATH'))
...
class Dashboard extends MY_Controller {
    ...
    function index($year = FALSE) {
        ...
        //var_dump($this->view_data["projects_open"]);exit;
        $this->view_data["projects_all"] = Project::count();
        // Invoices Stats	
        $this->view_data["invoices_open"] = Invoice::count(array('conditions' => array('status != ? AND status != ? AND estimate != ?', 'Paid', 'Canceled', 1))); // Get all but canceled and Paid invoices
        $this->view_data["invoices_all"] = Invoice::count(array('conditions' => array('status != ? AND estimate != ?', 'Canceled', 1))); // Get all but canceled invoices
        ...
        if ($this->user->admin == 0) {
            ...
            //var_dump($thisUserHasNoCompanies);exit;
            if (!empty($thisUserHasNoCompanies)) {
                ...
                $this->view_data['recent_activities'] = array(); //TODO
            } else {
