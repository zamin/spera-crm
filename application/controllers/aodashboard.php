...
class Aodashboard extends MY_Controller {
    ...
    function index($year=FALSE) {
        ...
        if (in_array("aoprojects", $this->module_permissions)) {
            //echo "fsf";exit;
            $sql = 'SELECT t.* from project_has_tasks t join project_assign_tasks at on t.id=at.task_id and t.project_id=at.project_id
join user_roles r on at.assign_user_id=r.user_id where t.status = "open" and r.company_id="'.$this->sessionArr['company_id'].'" and r.role_id="'.$this->sessionArr['role_id'].'" 
UNION
SELECT t.* FROM project_has_tasks t JOIN projects p ON t.project_id = p.id JOIN user_roles r ON t.user_id = r.user_id AND p.company_id = r.company_id WHERE t.status = "open" AND r.company_id = "'.$this->sessionArr['company_id'].'" AND r.role_id ="'.$this->sessionArr['role_id'].'" and t.user_id="'.$this->sessionArr['user_id'].'"';
        ...
        // Projects Stats
        $open_projects = Project::find_by_sql('Select count(*) as opcount from projects p left join user_roles r on p.company_id=r.company_id where r.user_id='.$this->sessionArr['user_id'].' and r.role_id='.$this->sessionArr['role_id'].' and r.company_id='.$sessionArr['company_id'].' and p.progress < 100 order by p.id desc');
        ...
        $all_projects = Project::find_by_sql('Select count(distinct(p.id)) AS "pcount" from projects p left join user_roles r on p.company_id=r.company_id where r.user_id = "' . $this->user->id . '" and r.role_id="' . $this->sessionArr['role_id'].'" and r.company_id="'.$this->sessionArr['company_id'].'" order by p.id desc');
