 ...
 * ClassName: cprojects
 * Function Name: index 
 * This class is used for client projects
 * */
if (!defined('BASEPATH'))
...
class Cprojects extends MY_Controller {
    ...
    function __construct() {
        ...
        $this->view_data['submenu'] = array(
            //$this->lang->line('application_my_projects') => 'cprojects/index/' . $this->sessionArr['company_id'],
            $this->lang->line('application_all') => 'cprojects/filter/all',
            $this->lang->line('application_open') => 'cprojects/filter/open',
            $this->lang->line('application_closed') => 'cprojects/filter/closed'
        );
    ...
    function filter($condition) {
        ...
        if ($this->user->admin == 0) {
            ...
            if (!empty($thisUserHasNoCompanies)) {
                ...
                //$projects_by_client_admin = Project::find_by_sql('SELECT * from projects p left join user_roles r on p.company_id=r.company_id where p.company_id="'.$this->sessionArr['company_id'].'" and r.role_id="'.$this->sessionArr['role_id'].'" and p.'.$options.'');
...
');
        ...
        //echo "<pre>";print_r($this->view_data['project']);exit;
        $this->content_view = 'projects/client_views/all';
    }
    ...
    function view($id = FALSE) {
        ...
        //echo "<pre>";print_r($this->view_data['project']->project_has_milestones);exit;
    ...
    function media($id = FALSE, $condition = FALSE, $media_id = FALSE) {
        ...
        switch ($condition) {
            case 'view':
                ...
                $this->view_data['form_action'] = base_url().'cprojects/media/' . $id . '/view/' . $media_id;
                $this->view_data['filetype'] = explode('.', $this->view_data['media']->filename);
                $this->view_data['filetype'] = $this->view_data['filetype'][1];
                $this->view_data['backlink'] = 'cprojects/view/' . $id.'#media-tab';
                break;
            case 'add':
                $this->content_view = 'projects/client_views/_media';
                $this->view_data['project'] = Project::find($id);
                if ($_POST) {
                    ...
                    redirect('cprojects/view/' . $id.'#media-tab');
                } else {
                    $this->theme_view = 'modal';
                    $this->view_data['title'] = $this->lang->line('application_add_media');
                    $this->view_data['form_action'] = base_url().'cprojects/media/' . $id . '/add';
                    $this->content_view = 'projects/client_views/_media';
                }
                break;
            case 'update':
                ...
                if ($_POST) {
                    ...
                    $this->view_data['form_action'] = base_url().'cprojects/media/' . $id . '/update/' . $media_id;
                    $this->content_view = 'projects/client_views/_media';
                }
                break;
            case 'delete':
    ...
    function tasks($id = FALSE, $condition = FALSE, $task_id = FALSE) {
        ...
        switch ($condition) {
            case 'comment':
                ...
                if(!empty($task))
                {
                    $this->content_view = 'projects/client_views/view_task_comment';
                    ...
                    $this->view_data['form_action'] = base_url().'cprojects/tasks/' . $id . '/view/' . $task_id;
                    $this->view_data['backlink'] = 'cprojects/view/' . $id.'#tasks-tab';
                }
                else
                {
                    redirect('cdashboard');
            ...
            case 'view':
                if($_POST) 
                {
                    ...
                    redirect('cprojects/tasks/' . $id . '/view/' . $task_id);
                }

                $task = ProjectHasTask::find_by_id($task_id);
                if(!empty($task))
                {
                    $this->content_view = 'projects/client_views/view_task';
                    $this->view_data['task'] = $task;
            ...
            case 'add':
                ...
                if ($_POST) {
                    ...
                    redirect('cprojects/view/' . $id.'#tasks-tab');
                } else {
                    $this->theme_view = 'modal';
                    $this->view_data['project'] = Project::find($id);
                    ...
                    {
                        $client_arr=array();
                        //echo 'SELECT u. * FROM user_roles r LEFT JOIN users u ON r.user_id = u.id WHERE r.company_id ="'.$this->sessionArr['company_id'].'" AND r.role_id = "2" and u.status="active"';exit;
            ...
            case 'update':
                ...
                if ($_POST) {
                    ...
                    $this->view_data['form_action'] = base_url().'cprojects/tasks/' . $id . '/update/' . $task_id;
                    $this->content_view = 'projects/client_views/_tasks';
                }
                break;
            case 'check':
            ...
            default:
                $this->view_data['project'] = Project::find($id);
                //var_dump( $this->view_data['project'] );exit;
                $this->content_view = 'projects/client_views/tasks';
                break;
        }
    }
    ...
    function timesheets($taskid) {
        ...
        $this->view_data['form_action'] = base_url().'cprojects/timesheet_add';
        $this->content_view = 'projects/client_views/_timesheets';
    }
    ...
    function milestones($id = FALSE, $condition = FALSE, $milestone_id = FALSE) {
        ...
        switch ($condition) {
            case 'comment':
                ...
                if(!empty($milestone))
                {
                    ...
                    $this->content_view = 'projects/client_views/view_comment';
                    ...
                    $this->view_data['form_action'] = base_url().'cprojects/milestones/' . $id . '/view/' . $milestone_id;
                    $this->view_data['backlink'] = 'cprojects/view/' . $id.'#milestones-tab';
                }
                else
                {
                    redirect('cdashboard');
            ...
            case 'view':
                if($_POST) 
                {
                    ...
                    redirect('cprojects/milestones/' . $id . '/view/' . $milestone_id);
                }
                ...
                {
                    $this->view_data['milestone'] = $milestone;
                    $this->content_view = 'projects/client_views/view_milestone';
                ...
                break;
            case 'add':
                $this->content_view = 'projects/client_views/_milestones';
                if ($_POST) {
                    //unset($_POST['send']);
                    //unset($_POST['files']);
                    //$description = $_POST['description'];
                    ...
                    redirect('cprojects/view/' . $id.'#milestones-tab');
                } else {
                    $this->theme_view = 'modal';
                    $this->view_data['project'] = Project::find($id);
                    $this->view_data['title'] = $this->lang->line('application_add_milestone');
                    $this->view_data['form_action'] = base_url().'cprojects/milestones/' . $id . '/add';
                    $this->content_view = 'projects/client_views/_milestones';
                }
                break;
            case 'update':
                ...
                if ($_POST) {
                    ...
                    $this->view_data['form_action'] = base_url().'cprojects/milestones/' . $id . '/update/' . $milestone_id;
                    $this->content_view = 'projects/client_views/_milestones';
                }
                break;
            case 'delete':
        ...
        $delete_mile_attachment=$this->db->query('DELETE From project_has_milestones_attachment where milestone_id="'.$milestone_id.'"');
                ...
                {
                    $delete_attachement_error= 'Error in delete milestone attachement';
                }
                $milestone = ProjectHasMilestone::find($milestone_id);
            ...
            default:
                $this->view_data['project'] = Project::find($id);
                $this->content_view = 'projects/client_views/milestones/';
                break;
        }
    }
    ...
    function delete_milestone_attachement($project_id=FALSE,$condition=FALSE,$milestone_id=FALSE,$milestone_attach_id=FALSE)
    {
        if(!empty($condition) && !empty($milestone_attach_id))
        {
            ...
            redirect('cprojects/view/' . $project_id.'#milestones-tab');
        }
    }
    ...
    function delete_tasks_attachement($project_id=FALSE,$condition=FALSE,$task_id=FALSE,$task_attach_id=FALSE)
    {
        if(!empty($condition))
        {
            ...
            redirect('cprojects/view/' . $project_id.'#tasks-tab');
        }
    }
    ...
    function task_list()
    {
        ...
        if($_GET['id'])
        {
            ...
            foreach ($newArr as $key => $value) {
                ...
                $new_html.= '<a href="'.base_url().'cprojects/task_start_stop_timer/'.$value['id'].'" data-timerid="timer'.$value['id'].'" class="grid__col-6 grid__col--bleed center ajax-silent task__options__button task__options__button--green task__options__timer timer'.$value['id'].''.$start.'">
                              '.$this->lang->line('application_start_timer').'</a>';
              $new_html.= '<a href="'.base_url().'cprojects/task_start_stop_timer/'.$value['id'].'" data-timerid="timer'.$value['id'].'" class="grid__col-6 grid__col--bleed center ajax-silent task__options__button task__options__button--red task__options__timer timer'.$value['id'].' hidden">
              '.$this->lang->line('application_stop_timer').'</a>';
              $new_html.= '<a href="'.base_url().'cprojects/tasks/'.$id.'/update/'.$value['id'].'" class="grid__col-6 grid__col--bleed task__options__button" data-toggle="mainmodal">
                ...
                $new_html.='<a href="'.base_url().'cprojects/timesheets/'.$value['id'].'" class="timer__icon_button tt timespentp" data-original-title="'.$this->lang->line('application_timesheet').'" data-toggle="mainmodal"><i class="ion-android-list"></i></a>';
                $new_html.='</li>';
                $new_html.='<li>';
