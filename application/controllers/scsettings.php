...
class Scsettings extends MY_Controller
{
	...
	function __construct()
	{
        ...
        //echo date('Y-m-d');die;
        //and pus.start_date <= `'.strtotime(date('Y-m-d')).'` and pus.end_date >= `'.strtotime(date('Y-m-d')).'`
        ...
        {	
            $this->view_data['submenu'] = array(
		 		$this->lang->line('application_enable_api') => base_url().'scsettings/apisettingview',
		 		);	
        }
        else
        {
	...
	function apisettingview()
	{
		...
		if($_POST['submit'] == 'Enable')
		{
            ...
            else
            {
            	...
            	$sql_check_api_details = 'SELECT * FROM user_api_details WHERE user_id = "'.$user_id.'"';
                $check_api_id = $this->db->query($sql_check_api_details)->row_array();
                $check_expired_time = $check_api_id['expired_login_date'];
			...
			redirect('scsettings/apisettingview');
		}
		elseif($_POST['submit'] == 'Disable')
		{
            ...
            if(!empty($check_api_id))
            {
	            ...
	            if(!empty($check_user_validation))
	            {
					...
					$this->view_data['id'] = $id;
					$this->view_data['access_token'] = $this->user_access_token;
					$this->view_data['login_token'] = $login_token;
		...
		else
		{
            ...
            {
                //echo '<pre>';print_r($this->check_ao_api_id);die;
                $user_id = $this->sessionArr['user_id'];
    			$sql_check_api_details = 'SELECT * FROM user_api_details WHERE user_id = "'.$user_id.'"';
                $check_api_id = $this->db->query($sql_check_api_details)->row_array();
		...
		$this->view_data['form_action'] = base_url().'scsettings/apisettingview';
		$this->content_view = 'settings/scsettings/enableapi'; 		
	}
