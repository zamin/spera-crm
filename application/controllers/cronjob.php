	...
	function index()
	{
		...
		if($core_settings->cronjob == "1" && time() > ($core_settings->last_cronjob + 300))
		{
			...
			$core_settings->save();
   			...
   			//Check Subscriptions
			$sql = 'SELECT * FROM subscriptions WHERE status != "Inactive" AND (end_date > "'.$date.'" OR end_date = "") AND "'.$date.'" >= next_payment ORDER BY next_payment';
						       		...
						       		if($invoice){	
							    		...
							    		$filename = $this->lang->line('application_invoice').'_'.$data["invoice"]->reference;
							     		...
							     		$due_date = date($data["core_settings"]->date_format, human_to_unix($data["invoice"]->due_date.' 00:00:00')); 		
							  			...
							  			$mail_subject = $this->parser->parse_string($data["core_settings"]->invoice_mail_subject, $parse_data);
							  			//email
