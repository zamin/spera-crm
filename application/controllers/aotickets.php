...
class Aotickets extends MY_Controller {
	...
	function index()
	{
		...
		$this->content_view = 'tickets/ao_view/all';
	}
	function queues($id)
	{
	...
	{	
		if($_POST){
            ...
            if (!empty($assign_user_id)) 
            {
       		...
       		if(!$ticket){$this->session->set_flashdata('message', 'error:'.$this->lang->line('messages_create_ticket_error'));
				...
				//redirect('aotickets/view/'.$ticket->id);
                redirect('aotickets');
       		}
		...
		}else
		{
			...
			$this->view_data['form_action'] = base_url().'aotickets/create';
            $this->content_view = 'tickets/ao_view/_ticket';
		}	
	}	
    ...
    function update($id = FALSE) {
       /* $t_assign_query_new = 'SELECT assign_user_id from project_assign_clients where project_id="' . $id . '"';
        ...
        if($_POST){
            ...
            $_POST['updated'] = "1";
            $_POST['subject'] = htmlspecialchars($_POST['subject']);
            $_POST['reference'] = $ticket->reference;
            $_POST['status'] = $ticket->status;
            ...
            $this->theme_view = 'modal';
            $this->view_data['title'] = $this->lang->line('application_create_ticket');
            $this->view_data['form_action'] = base_url().'aotickets/update/'.$id;
            $this->content_view = 'tickets/ao_view/_ticket';
        }
    }
	...
	{	
		...
		}else
		{
			...
			$this->view_data['form_action'] = base_url().'aotickets/assign';
	...
	{	
		...
		}else
		{
			...
			$this->view_data['form_action'] = base_url().'aotickets/queue';
	...
	{	
		...
		}else
		{
			...
			$this->view_data['form_action'] = base_url().'aotickets/type';
	...
	{	
		...
		}else
		{
			...
			$this->view_data['form_action'] = base_url().'aotickets/status';
	...
	{	
		...
		}else
		{
			...
			$this->view_data['form_action'] = base_url().'aotickets/close';
	...
	{ 
		...
		$this->content_view = 'tickets/ao_view/view';
		$this->view_data['ticket'] = Ticket::find_by_id($id);
		...
		$this->view_data['form_action'] = base_url().'aotickets/article/'.$id.'/add';
	...
	function article($id = FALSE, $condition = FALSE, $article_id = FALSE)
	{
		...
		switch ($condition) {
			case 'add':
				$this->content_view = 'tickets/ao_view/_note';
				if($_POST){
					$config['upload_path'] = './files/media/';
					$config['encrypt_name'] = TRUE;
					$config['allowed_types'] = '*';
	...
	{	
        ...
        if($_POST){
            ...
            if(empty($_POST['bulk'])){redirect('aotickets');}
