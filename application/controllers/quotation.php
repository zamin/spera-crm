...
class Quotation extends MY_Controller {
	...
	function index()
	{
		if($_POST){
			...
			$item = Quote::create($_POST);
       		if(!$item){$this->session->set_flashdata('message', 'error:'.$this->lang->line('quotation_create_error'));}
       				...
       				$company_detail = CompanyDetails::find('all', array( 'conditions' => array('company_id=?',$this->sessionArr['company_id']) ));
					$this->view_data['company_detail'] = $company_detail[0];
					//print_r($this->view_data['company_detail']);die;
					...
					//send_notification($value->email, $this->lang->line('application_notification_quotation_subject'), $this->lang->line('application_notification_quotation'));
				        ...
				        $this->email->subject($this->lang->line('application_notification_quotation_subject'));
				        $this->email->message($this->lang->line('application_notification_quotation'));
				        if($this->email->send()) {
		...
		}else
		{
			...
			//echo '<pre>';print_r($this->view_data['company_detail']);die;
			$this->view_data['form_action'] = base_url().'quotation';
	...
	function qid($id = FALSE)
	{
		...
		if($_POST){
			...
			$attributes = array('form' => $form, 'status' => 'New', 'custom_quotation_id' => $id, 'date' => date("Y-m-d H:i", time()), 'user_id' => $this->user->id);
		...
		$this->view_data['form_action'] = base_url().'quotation/qid/'.$id;
