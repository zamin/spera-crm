...
class Sctickets extends MY_Controller {
	...
	function __construct()
	{
		...
		//$this->view_data['tickets_assigned_to_me'] = TicketAssign::count(array('conditions' => 'user_id = '.$this->sessionArr['user_id']));
		//$this->view_data['tickets_in_my_queue'] = Ticket::count(array('conditions' => 'user_id = '.$this->sessionArr['user_id'].' and status != "closed"'));        
	...
	function index()
	{
		...
		$this->content_view = 'tickets/sc_view/all';
	...
	{	
		if($_POST){
            ...
            if (!empty($assign_user_id)) 
            {
       		...
       		if(!$ticket){$this->session->set_flashdata('message', 'error:'.$this->lang->line('messages_create_ticket_error'));
				...
				//redirect('sctickets/view/'.$ticket->id);
                redirect('sctickets');
       		}
		...
		}else
		{
			...
			$this->view_data['form_action'] = base_url().'sctickets/create';
            $this->content_view = 'tickets/sc_view/_ticket';
		}	
	}	
    ...
    function update($id = FALSE) {
       /* $t_assign_query_new = 'SELECT assign_user_id from project_assign_clients where project_id="' . $id . '"';
        ...
        if ($_POST) {
            ...
            $this->theme_view = 'modal';
            $this->view_data['title'] = $this->lang->line('application_create_ticket');
            $this->view_data['form_action'] = base_url().'sctickets/update/'.$id;
            $this->content_view = 'tickets/sc_view/_ticket';
        }
    }
	...
	{	
		...
		}else
		{
			...
			$this->view_data['form_action'] = base_url().'sctickets/assign';
	...
	{	
		...
		}else
		{
			...
			$this->view_data['form_action'] = base_url().'sctickets/queue';
	...
	{	
		...
		}else
		{
			...
			$this->view_data['form_action'] = base_url().'sctickets/type';
	...
	{	
		...
		}else
		{
			...
			$this->view_data['form_action'] = base_url().'sctickets/status';
	...
	{	
		...
		}else
		{
			...
			$this->view_data['form_action'] = base_url().'sctickets/close';
	...
	{ 
		...
		$this->content_view = 'tickets/sc_view/view';
		$this->view_data['ticket'] = Ticket::find_by_id($id);
		...
		$this->view_data['form_action'] = base_url().'sctickets/article/'.$id.'/add';
	...
	function article($id = FALSE, $condition = FALSE, $article_id = FALSE)
	{
		...
		switch ($condition) {
			case 'add':
				$this->content_view = 'tickets/sc_view/_note';
				if($_POST){
					$config['upload_path'] = './files/media/';
					$config['encrypt_name'] = TRUE;
					$config['allowed_types'] = '*';
    ...
    {    
        ...
        if($_POST){
            ...
            if(empty($_POST['bulk'])){redirect('sctickets');}
