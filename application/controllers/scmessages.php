 ...
 * This class is used for Subcontractor messages
 **/
if (!defined('BASEPATH'))
    exit('No direct script access allowed');

class Scmessages  extends MY_Controller {
  ...
  function __construct()
    ...
    {   
        //echo 'reach';die;
        $this->content_view = 'messages/subcontractor/all';
    }
    function messagelist($con = FALSE, $deleted = FALSE)
    {
        ...
        $this->content_view = 'messages/subcontractor/list';
    }
    ...
    {   
        if($_POST){
            ...
            if($ajax != "reply"){ redirect('scmessages'); }
            else{
                    $this->theme_view = 'ajax';
                }
            redirect('scmessages/');
        }else
        {
            ...
            $this->view_data['form_action'] = base_url().'scmessages/write';
            $this->content_view = 'messages/subcontractor/_messages';
        }   
    }   
    ...
    {   
        if($_POST){
            ...
            if(isset($view)){redirect('messages/subcontractor/view/'.$id);}else{redirect('messages');}
        ...
        }else
        {
            ...
            $this->view_data['form_action'] = 'messages/subcontractor/update';
    ...
    function view($id = FALSE, $filter = FALSE, $additional = FALSE)
    {
        $this->view_data['submenu'] = array(
                        $this->lang->line('application_back') => 'scmessages',
                        );  
        $message = Privatemessage::find_by_id($id);
        //echo $filter; 
        ...
        //echo 'reach';die;
            ...
            $sql2 = 'SELECT privatemessages.id, privatemessages.`status`,privatemessages.`deleted`, privatemessages.conversation, privatemessages.attachment, privatemessages.attachment_link, privatemessages.subject, privatemessages.message, privatemessages.sender,privatemessages.receiver_delete, privatemessages.recipient, privatemessages.`time`, privatemessages.`sender` , u1.`userpic` as userpic_c, users.`userpic` as userpic_u , users.`email` as email_u , u1.`email` as email_c , CONCAT(users.firstname," ", users.lastname) as sender_u, CONCAT(u1.firstname," ", u1.lastname) as sender_c, CONCAT(rec_u.firstname," ", rec_u.lastname) as recipient_u, CONCAT(rec_c.firstname," ", rec_c.lastname) as recipient_c
                    ...
                    GROUP by privatemessages.id HAVING privatemessages.id = "'.$id.'" OR privatemessages.conversation = "'.$row->conversation.'" ORDER BY privatemessages.`id` DESC LIMIT 100';
            $query2 = $this->db->query($sql2);
        ...
        else
        {
            ...
            if($filter == 'Deleted')
            {
            	...
            	$sql = 'SELECT privatemessages.id, privatemessages.`status`, privatemessages.`deleted`, privatemessages.conversation, privatemessages.attachment, privatemessages.attachment_link, privatemessages.subject, privatemessages.message, privatemessages.sender,privatemessages.receiver_delete, privatemessages.recipient, privatemessages.`time`, privatemessages.`sender` , clients.`userpic` as userpic_c, users.`userpic` as userpic_u , users.`email` as email_u , clients.`email` as email_c , CONCAT(users.firstname," ", users.lastname) as sender_u, CONCAT(clients.firstname," ", clients.lastname) as sender_c, CONCAT(users.firstname," ", users.lastname) as recipient_u, CONCAT(clients.firstname," ", clients.lastname) as recipient_c
                    FROM privatemessages
                    LEFT JOIN users AS clients ON (CONCAT("u",clients.id) = privatemessages.sender)
                    LEFT JOIN users ON (CONCAT("u",users.id) = privatemessages.sender) 
                    GROUP by privatemessages.id HAVING privatemessages.id = "'.$id.'" OR privatemessages.recipient = "u'.$this->user->id.'" ORDER BY privatemessages.`id` DESC LIMIT 100';
        ...
        $this->theme_view = 'ajax';
        $this->view_data['form_action'] = 'scmessages/write';
        $this->view_data['id'] = $id;
        $this->content_view = 'messages/subcontractor/view';
    }
}
