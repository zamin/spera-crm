...
class Subscriptions extends MY_Controller {
	...
	function index()
	{
		$subscriptions = Subscription::find_by_sql('SELECT u.*, (select id from propay_user_subscription as pus where pus.user_id = u.id order by pus.id DESC limit 1) as propay_user_subscription_id,
                                                                (select start_date from propay_user_subscription as pus where pus.user_id = u.id order by pus.id DESC limit 1) as propay_user_subscription_start_date,
                                                                (select end_date from propay_user_subscription as pus where pus.user_id = u.id order by pus.id DESC limit 1) as propay_user_subscription_end_date,
                                                                (select payment_detail_id from propay_user_subscription as pus where pus.user_id = u.id order by pus.id DESC limit 1) as propay_user_subscription_payment_detail_id
                                                    FROM users u
                                                    WHERE  (select count(*) from propay_user_subscription as pus where pus.user_id = u.id) > 0
                                                    order by u.id desc ');
        ...
        {   
            ...
            {
                $subscription_all[$i]['id']=$value->id;
                //$subscription_all[$i]['username']=$value->username;
                $subscription_all[$i]['name']=$value->firstname.' '.$value->lastname;
                //$subscription_all[$i]['lastname']=$value->lastname;
                ...
                $subscription_all[$i]['status']=$value->status;
