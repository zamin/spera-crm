    ...
    function index() {
        ...
        if (!empty($company_id)) {
            ...
            if ($client_query->num_rows() > 0) {
                ...
                //echo "<pre>";print_r($client_query->result_object());exit;
            }
        }
...
//            }
            ...
            //echo "<pre>";print_r($this->view_data['user']);exit;
            // $this->view_data['modules'] = Module::find('all', array('order' => 'sort asc', 'conditions' => array('type = ?', 'client')));
            ...
            if (!empty($company)) {
                ...
                if (!empty($newArr)) {
                    ...
                    foreach ($newArr as $newAr) {
                        ...
                        $content = '<a class="btn btn-danger po-delete ajax-silent" href="' . base_url() . 'clients/delete/' . $newAr['id'] . '" >' . htmlspecialchars($this->lang->line('application_yes_im_sure'), ENT_QUOTES) . '  </a><button class="btn po-close">' . $this->lang->line('application_no') . '</button><input type="hidden" name="td-id" class="id" value="' . $newAr['id'] . '">';
                        ...
                        $newhtml .= "<td class='hidden-xs'><a href='" . base_url() . 'clients/view/' . $newAr['id'] . "'>" . $newAr['firstname'] . "</a></td>";
                        $newhtml .= "<td class='hidden-xs'>" . $newAr['lastname'] . "</td>";
                        $newhtml .= "<td class='hidden-xs'><a href='" . base_url() . 'clients/view/' . $newAr['id'] . "'>" . $newAr['email'] . "</a></td>";
                        $newhtml .= "<td class='hidden-xs'>" . $newAr['status'] . "</td>";
                        if (!empty($newAr['company_name'])) {
                            $newhtml .= "<td class='option option-left'><a href='" . base_url() . 'clients/edit_company/' . $newAr['id'] . '/' . $newAr['sub_id'] . "'  data-toggle='mainmodal'>" . $newAr['company_name'] . "</a></td>";
                        } else {
                        ...
                        if ($newAr['status'] == "active") {
                            $newhtml .= "<a href='" . base_url() . 'clients/update/' . $newAr['id'] . "' class='btn-option' data-toggle='mainmodal'><i class='fa fa-cog'></i></a>";
                ...
                $json_arr = array('total_pages' => $totalpages, 'count' => $count_elements, 'result' => $newhtml, 'pagination_links' => $pagination_links);
                ...
                if ($client_query->num_rows() > 0) {
                    ...
                    if (!empty($clients)) {
                        ...
                        foreach ($clients as $newAr) {
                            ...
                            $user_cnt++;
                            $content = '<a class="btn btn-danger po-delete ajax-silent" href="' . base_url() . 'clients/delete/' . $newAr['user_id'] . '" >' . htmlspecialchars($this->lang->line('application_yes_im_sure'), ENT_QUOTES) . '  </a><button class="btn po-close">' . $this->lang->line('application_no') . '</button><input type="hidden" name="td-id" class="id" value="' . $newAr['user_id'] . '">';
                            $main_content = "data-content='" . $content . "' data-original-title='<b>" . htmlspecialchars($this->lang->line('application_really_delete'), ENT_QUOTES) . "</b>'";
                            $newhtml .= "<tr id='" . $newAr['user_id'] . "'>";
                            $newhtml .= "<td class='hidden-xs' style='width:70px'>" . $core_settings->company_prefix . $user_cnt . "</td>";
                            $newhtml .= "<td class='hidden-xs'><a href='" . base_url() . 'clients/view/' . $newAr['user_id'] . "'>" . $newAr['firstname'] . "</a></td>";
                            $newhtml .= "<td class='hidden-xs'>" . $newAr['lastname'] . "</td>";
                            $newhtml .= "<td class='hidden-xs'><a href='" . base_url() . 'clients/view/' . $newAr['user_id'] . "'>" . $newAr['email'] . "</a></td>";
                            $newhtml .= "<td class='hidden-xs'>" . $newAr['status'] . "</td>";
                            ...
                            if (empty($get_client_assign_company)) {
                                ...
                                $newhtml .= "<td class='option option-left'><a href='" . base_url() . 'clients/edit_company/' . $newAr['user_id'] . '/' . $get_client_assign_company['id'] . "'  data-toggle='mainmodal'>" . $get_client_assign_company['sub_company'] . "</a></td>";
                            }
                            //echo "<pre>";print_r($get_client_assign_company);exit;
                            ...
                            $newhtml .= '<button type="button" class="btn-option delete po" data-toggle="popover" data-placement="left" ' . $main_content . '  ><i class="fa fa-times"></i></button>';
                            if ($newAr['status'] == "active") {
                                $newhtml .= "<a href='" . base_url() . 'clients/update/' . $newAr['user_id'] . "' class='btn-option' data-toggle='mainmodal'><i class='fa fa-cog'></i></a>";
                            } else {
    ...
    function edit_company($client_id = FALSE, $c_id = FALSE) {
        ...
        if ($_POST) {
            ...
            $this->view_data['form_action'] = base_url() . 'clients/edit_company';
    ...
    function search_companies() {
        if (!empty($_REQUEST)) {
            ...
            $subcontractor_query = $this->db->select('u.*,r.user_id')->from('users u')->join('user_roles r', 'r.user_id = u.id')->where("admin = '0' AND u.status IN ('active', 'inactive') AND r.role_id = 3 AND r.company_id = " . $company_id)->get()->result_array();
            $newhtml = "";
            if (!empty($subcontractor_query)) {
                //echo "<pre>";print_r($subcontractor_query);
                $get_subcontractor_query_client = $this->db->select('cc.client_id')->from('users u')->join('user_roles r', 'r.user_id = u.id')->join('client_companies cc', 'cc.client_id = r.user_id')->where("admin = '0' AND u.status IN ('active', 'inactive') AND r.role_id = 3 AND r.company_id = " . $company_id)->like('cc.sub_company', strtolower(trim($name_of_company)))->get()->result_array();
                ...
                if (empty($get_subcontractor_query_client)) {
                    ...
                    $main_subcontractor_query = $this->db->query('SELECT * FROM users u JOIN user_roles r ON r.user_id = u.id WHERE r.user_id NOT IN ("' . $implode_clients . '") AND admin = "0" AND u.status IN ("active", "inactive") AND r.role_id = 3 AND r.company_id = "' . $company_id . '"')->result_array();
                    //exit;
