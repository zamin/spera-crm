...
class Items extends MY_Controller {
	...
	function index()
	{
		...
		// Item::find('all',array('conditions' => array('inactive=?','0')));
	...
	function create_items(){
		if($_POST){
			...
			$item = Item::create($_POST);
       		if(!$item){
				...
				$this->db->insert('item_company', array('item_id' => $item->id, 'company_id' => $this->company_id));
			}
			redirect('items');
		...
		}else
		{
			...
			$this->view_data['form_action'] = base_url().'items/create_items';
	...
	function update_items($id = FALSE){
		...
		}else
		{
			...
			$this->view_data['form_action'] = base_url().'items/update_items';
			$this->content_view = 'invoices/_items';
		}	
	}
