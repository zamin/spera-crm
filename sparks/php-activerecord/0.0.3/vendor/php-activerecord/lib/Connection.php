...
abstract class Connection
{
    ...
    { 
        ...
        $user = explode(':', $url);
        $user_string = str_replace('//', '', $user[1]);

        $pass_exp = explode('@', $user[2]);
        $pass = str_replace(end($pass_exp), '', $user[2]); 
        $pass = rtrim($pass, "@");


        $host = explode('/', end($pass_exp));
        $host = $host[0];

        $db = explode('/', end($pass_exp));
        $db = rtrim($db[1], "charset=utf8");
        $db = rtrim($db, "?");
        
        
        $parts = array("scheme" => 'mysql', "host" => $host, "user" => $user_string, "pass" => $pass, "path" => "/".$db, "query" => "charset=utf8");
